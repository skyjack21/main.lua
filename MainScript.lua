-- [[ SKYJACK RBX v800: PRECISION-STRIKE - STABLE SPEED & GHOST NAME ]] --
...
local Names = {"STABLE SPEED", "WALL PASS", "INFINITE JUMP", "VIP PASS INJECT", "GHOST IDENTITY", "ANTI-BAN SHIELD"}
local Buttons = {}
local Index = 1

-- [[ 1. GHOST IDENTITY (HARD-RESET NAME) ]] --
-- Menggunakan loop super cepat untuk menghapus nametag sebelum sempat muncul di layar orang lain
        ...
        while Running do
            if Toggles.HideName and lp.Character then
                pcall(function()
                    ...
                    -- Menghancurkan tag dari sistem game (Overhead Gui)
            ...
            task.wait(0.1) -- Refresh Rate 10x per detik
        end
    end)
end

-- [[ 2. PHYSICS ENGINE: LINEAR STABILITY ]] --
rs.Heartbeat:Connect(function()
    ...
    if Toggles.Speed and hum.MoveDirection.Magnitude > 0 then
        ...
        -- SPEED STABILIZER: Menggunakan CFrame Lerping agar tidak nabrak/mundur
        -- Kecepatan dikurangi sedikit demi kestabilan tangga (0.35 - 0.5 range)
        local moveVelocity = hum.MoveDirection * 0.48
...
local function BuildUI()
    for _, v in pairs(pgui:GetChildren()) do if v.Name == "PRECISION_V800" then v:Destroy() end end
    Screen = Instance.new("ScreenGui", pgui)
    Screen.Name = "PRECISION_V800"
    ...
    LoginBtn.MouseButton1Click:Connect(function()
        ...
        if success then
            ...
            if data.KEY_LIST[KeyInput.Text] then
                ...
                StartInjections() GhostMode() Toggles.Shield = true
            end
        end
    end)
end
