-- [[ SKYJACK RBX v380: ULTRA-STABLE - FULL RE-ENGINEERED ]] --
...
-- [[ 1. THE GHOST ENGINE (FIXED HIDE NAME) ]] --
        ...
        while Running and char.Parent do
            if Toggles.HideName then
                pcall(function()
                    ...
                    -- Menghapus semua tag yang dipasang oleh script game (Head/Root)
...
-- [[ 2. PHYSICS CORE: HIP-HEIGHT STABILITY (NO JUMPING GLITCH) ]] --
rs.Heartbeat:Connect(function()
    ...
    -- SPEED & SLOPE STABILITY
    if Toggles.Speed and hum.MoveDirection.Magnitude > 0 then
        -- Gunakan HipHeight untuk "melangkah" otomatis di tangga/tanjakan tanpa melompat
        hum.HipHeight = 2.2 
        local moveTarget = (hum.SeatPart and hum.SeatPart.Parent:IsA("Model")) and hum.SeatPart.Parent.PrimaryPart or root
        moveTarget.CFrame = moveTarget.CFrame + (hum.MoveDirection * 0.45) -- Speed konstan & stabil
    else
        -- Kembalikan ke normal jika speed OFF atau diam
    ...
    if Toggles.WallPass then
        ...
        -- Reset Collision secara presisi
        for _, v in pairs(lp.Character:GetChildren()) do
...
local function StartInjections()
    ...
    task.spawn(function()
        while Running and task.wait(0.8) do
            if Toggles.Vip and lp.Character and root then
...
-- [[ 5. PREMIUM UI: COMPACT DESIGN ]] --
local function BuildUI()
    for _, v in pairs(pgui:GetChildren()) do if v.Name == "ULTRA_STABLE_V380" then v:Destroy() end end
    Screen = Instance.new("ScreenGui", pgui)
    Screen.Name = "ULTRA_STABLE_V380"
